...
> **ADDENDUM (same day, quick 260722-v01, commits a67c9ff · d634686 · a260447 · ca3574d):**
> Items 1–9 below are now CLOSED. (1) AppSidebar collapses below `md` into a Masthead-hamburger off-canvas drawer (`SidebarNav` extracted + `MobileNavDrawer`, backdrop/Escape/route-change close); `<main>` is `p-4 md:p-6` and a flex column; workspace stage tabs become a horizontally scrollable row below `lg`. (2) My Tasks groups by severity (Must fix → Review → rest, counts in headers). (3) Additive `runs.listRecentForWorkspace` (`take(100)`) added AND deployed to dev:modest-magpie-797 via `dev:once`; RunsTable switched to it; `listForWorkspace` untouched for the aggregate consumers. (4) The graph/AutoPublishBanner height chain is fixed (`<main>` flex-col + banner `shrink-0` + run-monitor shell `flex-1 min-h-0`). (5) RegistryTable capped at latest 50, IssueRevenueTable at latest 24, both with Show-all + "showing latest" notes. (6) Story stage now mounts ONE DecisionLog (was ~11). (7) HelpTip portals to `document.body` (fixed, viewport-clamped, flip-above, z-[60]) — no overflow-ancestor clipping anywhere. (8) ClaimsChecklist.tsx deleted. (9) Run Monitor fully migrated to the 1c token system (all `runs/_components`, graph components, layout, both run-detail pages, PreviewIframe — `grep neutral-` over run-monitor returns nothing).
> Also fixed (user-reported same session): the Draft stage's "Show preview" iframe collapsed to ~150px — the mount wrapper had only `min-h`/`flex-1` (no definite height), so PreviewIframe's `h-full` chain resolved to auto. Wrapper now `h-[max(560px,calc(100vh-280px))]` with a card border.
> Still open: item 10 (the `record-session` tooling bug — avoid the command in this repo).
