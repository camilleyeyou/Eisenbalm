...
- [x] **WBN-03**: A failed run shows a plain-language recovery rail — what happened / what completed successfully / what did not happen / recommended recovery — with Restart from this step (reusing completed steps, not re-paying) and Improve this agent; downstream steps dim as Skipped.
- [x] **WBN-04**: Agent Instructions shows *why a draft instruction exists*, linking back to the specific issue output that motivated it.
...
- [x] **READ-07**: Editor can move to the previous or next section without returning to the issue.
