...
| 2 | Snapshot written + confirmed BEFORE `graph.ainvoke()`; mid-run edit doesn't change in-flight behavior | CFG-04 | pytest: ordering test patching `eisenbalm_pipeline.api.runs.snapshot_config` (the api.runs binding) + `asyncio.create_task`, asserting the snapshot mock WAS called AND awaited BEFORE create_task; a resume-no-resnap test asserting snapshot is never called on resume | unit (pytest) |
...
| 02-T2 promptVersions.ts + runs.ts (create full required fields + setConfigSnapshot) | 22-02 | 1 | CFG-02/04 | typecheck+codegen+grep | `pnpm --filter @eisenbalm/convex typecheck && pnpm --filter @eisenbalm/convex codegen` (+ grep `status:`/`startedAt:`/`Date.now()`/`triggerSource` in convex/runs.ts proving the runs:create insert is schema-complete) | ✅ schema stub | ⬜ pending |
...
| 05-T1 run_weekly snapshot-before-task wiring (+ runs:create triggerSource) | 22-05 | 3 | CFG-04 | ast/grep | `cd packages/pipeline && uv run python -c "s=open('src/eisenbalm_pipeline/api/runs.py').read(); assert s.count('snapshot_config(')==1 and 'load_run_config' in s and 'runs:create' in s and 'triggerSource' in s"` | ✅ runs.py | ⬜ pending |
| 05-T2 11 call-site swap | 22-05 | 3 | CFG-01 | unit + grep | `cd packages/pipeline && uv run pytest tests/test_voice.py tests/test_section_writer_voice_propagation.py -q` (byte-equivalence) + grep acceptance: 11 sites read `state["config"].agents[...]`, no `.format(` in prompt assembly. NOTE: verification is grep + voice-pytest, NOT a named `test_call_site_swap` (the RESEARCH-doc reference to that name is superseded by this grep-based check). | ✅ agent files | ⬜ pending |
| 05-T3 snapshot-ordering tests + full suite | 22-05 | 3 | CFG-01/04 | unit | `cd packages/pipeline && uv run pytest tests/api/test_runs_config_snapshot.py -q && uv run pytest -q` (test patches `eisenbalm_pipeline.api.runs.snapshot_config` and asserts the mock WAS called before create_task) | ❌ 01 scaffolds, 05 greens | ⬜ pending |
