...
api/content.py:
  patch_section  (~line 234) — replaces a section body array; after mutation calls
      `await _revoke_active_signoffs(convex_http, run_id=run_id, reason="section edited")` (~line 278)
  patch_bonus    (~line 494) — variant-shaped; specAd branch handles `body.blocks` (~line 527);
      calls `_revoke_active_signoffs(..., reason="bonus edited")` (~line 562)
  bigBudget/jingle bonus.body is a PLAIN STRING (exempt per §35.3 D-06) — do NOT reset for those variants.
  The endpoint already reads a "before" state for _emit_audit — reuse that pre-patch body array as before_blocks.
...
<task type="auto" tdd="true">
  ...
  <action>
Add two module-level async helpers to api/content.py (transcribe the 42-RESEARCH sketch lines 376-400, adapting the Convex helper names to whatever content.py already imports for query/mutation — likely `convex_query`/`convex_mutation_safe`):
...
Note the block-text accessor: portable-text blocks store text differently — use whatever text projection content.py / the audit path already uses to compare block prose (mirror `_emit_audit`'s before/after body serialization); if blocks are `{_type:'block', children:[{text}]}`, compare a joined children-text string, not a top-level `.text`. Encode that concretely so the diff is on visible prose.

Add pure unit tests to packages/pipeline/tests/test_content_patch_endpoints.py (new test cases, name them `test_touched_block_indices_*` and `test_reset_touched_claims_*`) covering the <behavior> list, monkeypatching the Convex query/mutation calls to capture which claimIndexes got markChanged.
  </action>
  <verify>
...
<task type="auto" tdd="true">
  ...
  <files>packages/pipeline/src/eisenbalm_pipeline/api/content.py, packages/pipeline/tests/test_content_patch_endpoints.py</files>
  <read_first>
    - packages/pipeline/src/eisenbalm_pipeline/api/content.py (patch_section ~line 234 incl. its _revoke_active_signoffs call ~line 278; patch_bonus ~line 494 incl. the specAd branch ~line 527 and its _revoke_active_signoffs call ~line 562)
  ...
  <behavior>
    - patch_section, after its existing _revoke_active_signoffs call, computes touched = _touched_block_indices(before_body, new_blocks) and calls _reset_touched_claims(run_id, sectionName, touched) with the section's galley-id sectionName.
    - patch_bonus's specAd branch does the same for the bonus section (sectionName 'bonus'); the bigBudget and jingle branches do NOT call it (plain-string body, exempt).
...
```python
touched = _touched_block_indices(before_body, new_blocks)
await _reset_touched_claims(convex_http, run_id=run_id, section_name=<galley section name for this endpoint>, touched=touched)
```
Use the same `before` body array the endpoint already fetched for the audit and the same new blocks array it just patched. Resolve the section galley-id from the endpoint's existing section identifier (the value already written to claim_checks.sectionName by the publisher — originStory/problemStatement/founderBio/caseStudy).

In patch_bonus, inside the `if body.variant == "specAd":` branch ONLY, after its _revoke_active_signoffs call, add the same two lines with `section_name="bonus"` and the specAd before/after blocks. Leave bigBudget/jingle branches unchanged.
...
Add integration test cases to test_content_patch_endpoints.py per the <behavior> list, asserting the exact claimIndexes markChanged is called with, and asserting the bonus bigBudget path issues zero markChanged calls.
  ...
  <done>Editing any of the four long-read sections or the specAd bonus resets the touched-block claims to unchecked + sets the changed marker, alongside the existing sign-off revocation; string-body bonus variants are correctly exempt.</done>
</task>
...
<success_criteria>
FCT-07 satisfied: a revision touching a claim's block returns that claim to unchecked and increments changedCount even when the replacement text is itself sourced; index drift is handled conservatively (over-reset, never under-reset).
</success_criteria>
