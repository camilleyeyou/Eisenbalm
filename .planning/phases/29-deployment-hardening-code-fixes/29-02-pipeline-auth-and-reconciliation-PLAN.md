...
<task type="auto">
  ...
  <action>
    ...
    Wire into api/main.py lifespan on the CLEAN-boot path only, AFTER `convex_client.set_client(convex_http)` and BEFORE `yield`: `await reconcile_orphaned_runs(convex_http)` inside the try block (so it degrades with the rest of lifespan).
    **IMPORTANT (fixes plan-checker advisory 1): `assert_deployed_secrets()` (from Task 1) must be called at the VERY TOP of `lifespan`, BEFORE and OUTSIDE the blanket `try/except Exception` that swallows boot failures into degraded mode.** If it were called inside that try block, a genuinely misconfigured deployed process would have its fatal error caught and logged as "expected for local dev," then boot healthy anyway — defeating D-2's boot-time intent. Placing the call before the swallowing try makes the raise actually prevent a healthy boot in a deployed env. Note the per-request guards from Task 1 (`auth.py`/`control.py`/`agents.py`/`runs.py`) remain the primary enforcement and independently fail closed on every request; the boot assertion is defense-in-depth and must be effective, not swallowed. Do NOT place `assert_deployed_secrets()` inside the degraded-mode `try`.
