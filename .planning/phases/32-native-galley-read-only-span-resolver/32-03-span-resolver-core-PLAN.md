...
<task type="auto" tdd="true">
  ...
  <action>
    ...
    5. If 0 exact matches → run the normalization fallback in two length-safe stages. **Stage A (quote substitution — length-preserving):** define `normQuotes(s)` = map `’‘` → `'` and `“”` → `"` ONLY (1:1 char substitutions, string length unchanged). Recompute matches on `normQuotes(block.text).indexOf(normQuotes(quoted))`. Because lengths are unchanged, the matched index maps 1:1 onto the ORIGINAL `block.text` — use it directly as `start`, `end = start + quoted.length` after quote-normalizing `quoted` to the block's length (same length either way). **Stage B (whitespace tolerance — length-changing, so NEVER reuse normalized indexes):** if Stage A finds 0 matches, build a regex from the quote-normalized `quoted` by escaping regex metacharacters and replacing every `\s+` run with the pattern `\s+`, then execute it against `normQuotes(block.text)` via `RegExp.exec` — `match.index` and `match[0].length` are offsets/lengths in the ORIGINAL block text (quote substitution preserved length), so `start = match.index`, `end = match.index + match[0].length`. Count ALL regex matches per block (loop `exec` with a global flag) and apply the SAME single/hint/ambiguous rules from steps 3–4 across blocks. **Invariant (MUST hold and be asserted in tests): `block.text.slice(start, end)`, after `normQuotes` + whitespace-collapse on both sides, equals the normalized `quotedSpan`.** `start`/`end` ALWAYS index the original `block.text`; never return offsets into any normalized string. Keep normalization NARROW — never fuzzy/Levenshtein.
  ...
  <acceptance_criteria>
    ...
    - Normalization cases assert offset correctness against the ORIGINAL text: `block.text.slice(start, end)` (quote/whitespace-normalized) equals the normalized `quotedSpan` — a resolved annotation can never slice the wrong substring
