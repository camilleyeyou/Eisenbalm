...
<task type="auto" tdd="true">
  ...
  <behavior>
    - A finding whose span overlaps a tracked claim shows, in the same popover as its reason: the claim text, its source URL, its supporting passage and its retrieved date.
...
const findingClaimMap = useMemo(
  () => (showClaimEvidenceInFindings ? buildFindingClaimMap(resolved, claimResolved ?? []) : new Map()),
  [showClaimEvidenceInFindings, resolved, claimResolved],
)
```
...
**`Galley.tsx`** — add the same optional `showClaimEvidenceInFindings?: boolean` to `GalleyProps`, destructure it, and forward it on BOTH `<GallerySection …>` mounts (the `LONG_READ_SECTIONS` map and the `bonus`/specAd mount). Undefined leaves Review Desk and Voice Pass byte-identical (D-24).
...
Note the interaction with D-09 and state it in a code comment: `markSourcedClaims={false}` stops sourced claims being MARKED as spans, but `resolveClaimsFor` still returns unsourced ones, and a finding sitting on an unsourced claim is exactly the case where the editor most needs the evidence card ("this sentence has no source"). If a finding overlaps a claim that was filtered out by D-09, no card renders — that is correct and honest, not a bug.
  </action>
  <verify>
