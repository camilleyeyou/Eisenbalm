...
stopped_at: Completed quick 260721-qdx (add first-run onboarding to dispatch-control)
...
- [Quick 260721-qdx] New `user_onboarding` Convex table + `userOnboarding.ts` (byClerkUserId/completeTour/dismissCard/dismissStageHint) are NOT DEPLOYED — requires `pnpm --filter @eisenbalm/convex codegen && pnpm --filter @eisenbalm/convex dev:once` against dev:modest-magpie-797 (no Convex credentials in the execution worktree). `convex/_generated/api.d.ts` was hand-updated to the shape a real codegen run would produce (typecheck + full dispatch-control suite + strict build all pass against it), but the live deployment has no `userOnboarding` module until the sync runs — every `api.userOnboarding.*` call 404s/500s until then. See 260721-qdx-SUMMARY.md.
