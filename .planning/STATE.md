...
- v3.0 roadmap created (2026-07-06): milestone Dispatch Control v2 — Editorial Operator Console. 43 requirements (CHR/GLY/EDT/PUB/VOX/PRV/MON/SIG/EVL/MEM) mapped to 10 new phases, numbered 30-39 (continuing from Phase 29).
- v4.0 roadmap created (2026-07-14): milestone Dispatch Control v3 — The Editorial Workspace. 62 requirements (ISS/WSP/FCT/TSK/INS/REV/SGE/BRF/ENT/ROL/WBN) mapped one requirement-block-per-phase across 11 new phases, numbered 40-50 (continuing from Phase 39). Sequencing follows the binding spec's foundation-first order and PROJECT.md's locked decisions: **Phase 40** (issue entity + Issues home) inverts run-keyed routing to issue-keyed first, since everything else sits on it; **Phase 41** (Issue Workspace frame — Stages 2/4/5) is the first demoable milestone and is deliberately RECOMPOSITION over shipped v3.0 endpoints (galley, content-patch, sign-offs, provenance, span resolver) — not new backend; **Phase 42** (Fact Check) is the only genuinely new stage, built on the Phase 35 provenance substrate plus one new Researcher `importance` field; **Phases 43-45** (My Tasks/Decision log, universal inspector, agent revision) are cross-cutting workspace mechanics sequenced after Fact Check because they project over/reference its claims; **Phase 46** (Signal Editor + verify_candidates, 18→20 pipeline nodes) is a self-contained backend track with no console dependency, sequenced before Stage 1 because Stage 1 cannot render without its leads/verification records; **Phase 47** (Story & Brief stage) depends on Phase 46's output; **Phase 48** (brief entry point) depends on Phase 47's editable Brief artifact; **Phase 49** (roles/permissions) gates the six actions built across Phases 41/42/45/47; **Phase 50** (Workbench rename + nomenclature pass) closes the milestone since its terminology changes ripple across every screen built before it. Reuse discipline carried into every phase: the 1c design system (Phase 30), the two-sign-off publish gate (Phase 34 `sign_offs`), the eval commit gate (Phase 38), and the provenance substrate (Phase 35 `claim_checks`) are the foundation, not v4.0 work — no phase rebuilds them. Deferred nothing new; V3-DEF-01 (Sanity removal) and V3-DEF-03 (inline WYSIWYG) remain deferred from v3.0. Sequencing follows `.planning/research/SUMMARY.md`'s dependency-driven order and PROJECT.md's locked decisions: **Phase 30** (design system/chrome/inbox + the `NEXT_PUBLIC_PIPELINE_URL` fix) ships first as the lowest-risk shell everything else builds inside; **Phase 31** (content-patch endpoint family + full editing) is the load-bearing write-boundary change every later content-mutating feature depends on; **Phase 32** (native galley, read-only, + span-resolver) ships in PARALLEL with the existing preview iframe, not replacing it, de-risking the milestone's highest-complexity item; **Phase 33** (accept-fix wiring + decision rail) connects 31+32 and mechanically de-risks Voice Pass; **Phase 34** (two-sign-off publish gate + Studio bypass retirement) is the highest-risk single change — the webhook handler itself is re-validated, not just the dashboard button, and Studio lockdown ships in the SAME phase, not deferred; **Phase 35** (provenance pipeline) is deliberately sequenced after the galley exists so pipeline-side work gets an immediate frontend target, and claim IDs are established at LLM-generation time (never post-hoc fuzzy-matched, per the citation-hallucination pitfall); **Phase 36** (Voice Pass) follows accept-fix because it's mechanically the same span-match → popover → content-patch shape; **Phase 37** (Run Monitor v2 + Signal Desk) and **Phase 38** (Prompt Lab evals + Eval Center) are independent parallel tracks with zero schema/endpoint dependency on the review-desk track (30-36) — staffed in parallel from day one; **Phase 39** (registry coverage-memory strip) is the smallest, least-dependent item and can land any time. Cross-cutting constraint carried into every write-path phase (31, 33, 34): ship with a tested fallback to the prior working path and a soak period before retiring it — this is a single-operator, hard-Thursday-deadline system with no second reviewer. Deferred out of v3.0 scope: Sanity removal (V3-DEF-01, own follow-up milestone), Signal Editor agent + REAL/OBSCURE/SPECIFIC/TELLABLE gates (V3-DEF-02), inline WYSIWYG (V3-DEF-03), assignable EIC seat (V3-DEF-04), Suno/NotebookLM API automation (V3-DEF-05).
...
- [Phase 41]: The Issue Workspace frame is a client layout.tsx wrapping every stage route in ONE WorkspaceStateProvider + rendering the 5 live stage tabs, status header, outline, context-panel shell, and relocated Hold/run-history controls — staying mounted across tab switches (41-06)
- [Phase 41]: Bare /issues/[n] is now a redirect-only Server Component: lastVisitedStage (D-03) else Draft/Story (D-04); the overview's Hold/Reopen + run-history moved into WorkspaceControls so no capability was lost (Pitfall 5) (41-06)
...
- [Phase 42]: PATCH's content-touching branch and evidence/apply both terminate via claimChecks:keepAsWritten (status defaults to checked) rather than a bespoke status, since D-08's locked chip vocabulary has no separate state for edited/kept/confirmed
...
- [Phase 45]: InspectorFooter Ask agent to revise flips LIVE only when firstProseExcerpt derives a real non-empty quotedText; RevisionFlow stays mounted by the Draft/Voice surface, never by the inspector, via a shared requestRevision channel on InspectorProvider
...
| 260710-fxx | Convex deploy-parity guard: report-only Node script (`convex/scripts/check-deploy-parity.mjs` + `pnpm check:convex-parity`) diffing pipeline `convex_query/mutation` string-literal calls against the live `dev:modest-magpie-797` `function-spec`; exit 0 all-present / 1 drift / 2 unreachable-SKIP. Prevents recurrence of the Phase 39 prod 500 (`charities:listRecentFeatured` committed but unsynced). Verified: exit 0 (45 called / 110 deployed), exit 1 on bogus-path probe (cleaned up), exit 2 on unreachable. | 2026-07-10 | 5895732 |  | [260710-fxx-convex-deploy-parity-guard-check-called-](./quick/260710-fxx-convex-deploy-parity-guard-check-called-/) |
| 260710-iwy | Fix two live Run-Monitor review-screen bugs: (1) preview iframe vanished ~30s after loading — stale-closure `useEffect([])` in `PreviewIframe.tsx` flipped a loaded preview to the error state; fixed to `[loaded]` + early-return so the timer cancels on load (+ vitest fake-timer test). (2) garbled Factual Claims — `lib/claims.py` extracted proper nouns from Title-Case headlines; now excludes h1–h4/blockquote blocks (nested + flat paths) + headline string-fields, rejects >5-word proper-noun runs, collapses overlaps to the longest, word-bounds context. Verified: dispatch-control build 0 + 512 vitest; pipeline pytest 526 passed. Logged a pre-existing `RE_NUMBER` `$`-drop bug (deferred). | 2026-07-10 | 432bc5a |  | [260710-iwy-fix-preview-iframe-stale-closure-timeout](./quick/260710-iwy-fix-preview-iframe-stale-closure-timeout/) |
| 260710-k8y | Implement the Prompt Lab Nomenclature Proposal in dispatch-control: newsroom-register rename pass across 8 components — group taxonomy (`Agent system prompts`→`Job Descriptions`, `User templates`→`Assignment Memos`, `Section guidance`→`Section Briefs`, `Shared assets`→`House Rules`, reordered House Rules first) with new per-group descriptors + 12 curated `AGENT_DISPLAY_NAMES` (disambiguating the two Editor agents); `TestRunPanel`→"Rehearsal" (modes: Sample week / Your own input / Replay a real run + per-mode descriptors, compare action "Draft vs. live"); `AssembledPreview`→"What the agent sees"; `PromptSaveDialog` now resolves "Save draft as v{n}" and REQUIRES a non-empty note (Confirm save disabled until typed); `PromptEditor`'s unknown-variable warning now reads "{token} isn't supplied by the pipeline — remove it or ask your developer to wire it"; `VersionHistoryPanel` badge/controls → LIVE / Make live / Restore this version. Slugs, routes, the `InputMode` enum, payload keys, and all Convex signatures unchanged (verified zero diff to docs/API_CONTRACTS.md) — labels + one required-note behavior change only. New `PromptSaveDialog.test.tsx` (5 tests) proves the required-note gate. Verified: dispatch-control build 0 + 517 vitest passed across all 3 tasks. | 2026-07-10 | e3bdb0d |  | [260710-k8y-implement-prompt-lab-nomenclature-propos](./quick/260710-k8y-implement-prompt-lab-nomenclature-propos/) |
| fast | Fix invisible Settings Slack/Email toggle switches: the shared shadcn `<Switch>` used shadcn semantic tokens (`bg-primary`/`bg-input`/`bg-background`/`ring-ring`) that aren't `--color-*` names, so Tailwind never generated those utilities and the track+thumb rendered colorless (invisible on the white card). Swapped `components/ui/switch.tsx` to 1c utilities — `bg-green` when on / `bg-faint` (taupe) when off / `bg-card` (white) thumb / `ring-cobalt` focus. Only `<Switch>` usage is NotificationSettings (the two toggles). Verified: dispatch-control build 0 + 517 vitest passed. | 2026-07-10 | c90f20b | — |
...
| 260711-iu2 | GameWriter agent retuned + given a real design brief. `lib/llm_config.py`: `MODEL_BY_AGENT["game"]` → `anthropic/claude-opus-4-8`, temperature 0.7→0.4 (top_p unchanged), new `MAX_TOKENS_BY_AGENT["game"]=24_000` — no other agent touched. `prompts/game.md` rewritten from a one-line prompt into a full game-design brief (mission-enacting-mechanic requirement rejecting generic theme-skins, winnable-with-meaning win/lose states, "game feel" — immediate feedback/visible progress/difficulty ramp/smooth motion, a "no AI slop" visual bar, explicit validator-trap call-outs for `top.`/`parent.`/`fetch(`/`localStorage`/etc., and an output contract); `{charity_name}`/`{VOICE_CONSTRAINTS}`/`{FORBIDDEN_CONSTRUCTS}` tokens + PROMPT START/END markers preserved verbatim. `prompts/game_user.md` gained one line instructing the model to design the mechanic from the mission before writing code. Convex: saved new INACTIVE v4 prompt versions for `agentKey` `game` and `game_user` in workspace `eisenbalm` (via `npx convex run ... --identity` against dev:modest-magpie-797 — succeeded, not auth-blocked) plus updated the `game` row in the `agents` dashboard table (model/temperature/top_p/max_tokens); v3 remains the live/active prompt until Andrew Rehearses + activates v4 in Prompt Lab (eval-gated by design, not bypassed). Full pipeline suite: 526 passed/36 skipped/0 failed — no regressions. | 2026-07-11 | 194c583 | | [260711-iu2-game-section-quality-opus-4-8-gamewriter](./quick/260711-iu2-game-section-quality-opus-4-8-gamewriter/) |
| 260717-35u | Fix FastAPI error-envelope unwrapping in the dispatch-control API clients. FastAPI serializes `HTTPException(detail={reason,message})` as `{"detail":{...}}`, but the clients read `body.reason` instead of `body.detail.reason` — so `reason` was always `'unknown'` and `message` always the literal `"HTTP 409"`, leaving all 11 reason-gated error branches in the dashboard dead. This is why an operator hit Publish 3x in production against an unexplained 409: the server was sending the exact remediation copy ("Both sign-offs (Facts cleared + Sounds human) are required") and the client discarded it. Scope was corrected during planning — 4 sites across 3 files (`reviewClient.ts:83`, `signOffClient.ts:77`, `contentPatchClient.ts` ×2), NOT all 8 clients; the other 5 were already correct, and one of them (`findingsClient.ts:103`) already carried the canonical unwrap idiom, which was adopted verbatim rather than inventing a variant. Also fixed the secondary `Array.isArray(detail?.fields)` bug at contentPatch site 1. New `__tests__/errorEnvelope.test.ts` (7 tests) exercises the REAL un-mocked parse path against FastAPI's true wire shape — the pre-existing tests mocked `ReviewApiError` directly, which is why the bug survived. RED gate independently reproduced on master (`expected 'unknown' to be 'missing_signoffs'`). Full suite 1031 passed; `pnpm --filter dispatch-control build` exits 0. No UI reason-vocabulary touched (`claims_not_signed_off` is still legitimately emitted by the facts-cleared sign-off endpoint). | 2026-07-17 | 7a04754 | | [260717-35u-fix-fastapi-error-envelope-unwrapping-ac](./quick/260717-35u-fix-fastapi-error-envelope-unwrapping-ac/) |
| 260717-41s | Make `charities:seedFromPublished` genuinely idempotent. It was documented as idempotent in three places (convex/charities.ts docstring, backfill_charity_registry.py:7 and :182) but did `timesFeatured: (existing.timesFeatured ?? 0) + 1`, so every re-seed inflated the counter — the live dev deployment showed `RIP Medical Debt` at `timesFeatured: 33886` and the Registry UI rendered "featured 33,886 times". Planning surfaced that the counter semantics were ALREADY meaningless, not just inflated: the caller name-deduped rows (backfill_charity_registry.py:111-114) before Convex saw them, so each run added exactly +1 per charity — `timesFeatured` was counting *backfill runs*, not issues. That also meant a mutation-only tally-and-SET fix would have computed 1 for every charity forever (idempotent but wrong), so the fix is necessarily three inseparable parts: (1) `seedFromPublished` rewritten as in-memory tally + SET (not increment), preserving dedupKey/domain derivation, status/lastFeaturedAt, backfill-if-missing semantics, `requirePipelineSecret`, sequential writes, and a tallied (not hardcoded-1) insert path; (2) caller dedup removed so multiplicity reaches the tally; (3) GROQ gained `!(_id in path("drafts.**"))` — the script authenticates with SANITY_API_TOKEN so draft copies carrying `status=="published"` would double-count once the dedup no longer masked them. `upsertFeatured` untouched (its per-publish increment is a correct, separate lane). New `__tests__/seedFromPublishedIdempotent.test.ts` (5 convex-test cases incl. a 33886 self-heal case); RED gate independently reproduced on master (`expected 2 to be 1`, `expected 33887 to be 1`). Convex typecheck exits 0; full suite 1036 passed. NOT DEPLOYED — requires `pnpm --filter @eisenbalm/convex dev:once` (no credentials in repo); deploy BEFORE the next backfill run, or old mutation + new caller inflates faster. Live 33886 row untouched; self-heals on the next backfill once deployed. | 2026-07-17 | a550a7b | | [260717-41s-make-convex-charities-seedfrompublished-](./quick/260717-41s-make-convex-charities-seedfrompublished-/) |
| 260721-pmn | Fix Approval-stage setPanelContent infinite render loop (dead clicks on /issues/[n]/approval, live prod outage): WorkspaceStateProvider handed out fresh identities every render (`claimRows` via `.map()`, `tasks`/`stages`/`status` via deriveTasks/deriveStageStates, `{}` signOffs fallback), so ApprovalPanelPublisher's useMemo→useEffect→setPanelContent cycle self-sustained (panelContent is provider state → re-render → new identities → new content → effect refires; passive-effect loop, no React max-depth throw, pegged main thread). Same latent loop in FactCheckPanelContent via `ws.claimRows`. Fix: useMemo the four identity-unstable derived values in the provider keyed on raw stable Convex results (single-point fix for both publishers; publisher effect contracts untouched; 260721-ohu's [runId]-only getDraft deps preserved). New bounded-settle regression `WorkspaceApprovalPanelLoop.test.tsx` (createRoot mount — RTL render() act-drains the sync loop to OOM; RED 49-57 renders, GREEN low single digits). Full suite 1054 passed; strict build exits 0. | 2026-07-22 | c3ba738 | | [260721-pmn-fix-approval-stage-setpanelcontent-infin](./quick/260721-pmn-fix-approval-stage-setpanelcontent-infin/) |
| 260721-qdx | Add first-run onboarding to dispatch-control: additive Convex `user_onboarding` table (sibling of `users`, keyed by clerkUserId) + `byClerkUserId`/`completeTour`/`dismissCard`/`dismissStageHint`; shell-level `OnboardingProvider` (single stable useQuery, one-shot auto-launch guard, independent from WorkspaceStateProvider) mounted in the dashboard layout; 5-step in-house `OnboardingTour` overlay (no new npm dep); dismissible `StartHereCard` on Issues home mapping all 5 stages via issueRouteResolver; per-stage `StageHintStrip` mounted above the stage canvas in each of the 5 workspace routes (proven via a comment-stripped source-scan to never touch useWorkspaceState/setPanelContent); `ReplayTourButton` on How to use. Fixed 6 pre-existing workspace-layout test harnesses that render IssueWorkspaceLayout in isolation (added an onboarding-context mock) after StageHintStrip's mount broke them. Full suite 1073 passed (134 files); strict build exits 0. NOT DEPLOYED to Convex dev — no credentials in the execution worktree; `convex/_generated/api.d.ts` hand-updated to the shape `codegen`/`dev:once` would produce (typecheck clean) Convex sync completed post-merge from the main checkout (codegen zero-diff + dev:once to dev:modest-magpie-797, smoke-verified) — no remaining blocker. | 2026-07-22 | 75f9d23 | | [260721-qdx-add-first-run-onboarding-to-dispatch-con](./quick/260721-qdx-add-first-run-onboarding-to-dispatch-con/) |
| 260722-f2c | Rename obscure Gate-1 operator-facing labels in dispatch-control's "Needs your decision" card to plain English (client-flagged jargon, extends Phase 47 D-07): "Rationale (required)"→"Why this pick? (required)" + new "Saved to the decision log — not published." helper line; "Evidence quality"→"What's been verified"; "Burden"→"What still needs checking"; `describeBurden`'s no-gaps copy→"Nothing left to check here."; error/success copy plainified ("Couldn't submit your choice — try again." / "Got it — the run is continuing with {pick}.") and synced verbatim to the signal-desk `AdjudicationPanel`'s two shared strings only (its own label/header/button untouched). No internal identifier, data-testid, or the `aria-label="Needs your decision"` region name changed. Caught and fixed an HTML-entity-in-plain-string bug before commit (`&rsquo;` doesn't decode in a runtime JS string rendered via `{message}`, only in compiled JSX text). 3 targeted test files (21 tests) + `pnpm --filter dispatch-control build` both green. | 2026-07-22 | afc958e | | [260722-f2c-rename-obscure-operator-facing-labels-in](./quick/260722-f2c-rename-obscure-operator-facing-labels-in/) |
| 260722-fom | Add help tooltips + inline helper text across all dispatch-control operator surfaces (client-requested "hover ? help on all features", executed as a hybrid after UX pushback on tooltip-everything): new in-house `HelpTip` primitive (components/ui/HelpTip.tsx — lucide HelpCircle, hover + keyboard-focus + tap-toggle, Escape close, aria-describedby, 44px touch target, viewport-clamped popover, zero new deps) + centralized `components/help/helpCopy.ts` (every help string keyed by its real rendering surface, plain operator English) + curated-density wiring across 15 surfaces (issue status chips/claim coverage/scheduled slot, workspace status+cost readouts+stage tabs, decision-card ranking, LeadActions require/remove, fact-check claim marks, VoicePassRail machine-tells+sign-offs, DecisionRail readiness board+publish, signal-desk adjudication+advocate score, runs status column, recovery rail, prompt-lab version history+eval override, notification settings). Consequential actions (Hold/Reopen, Remove lead, Publish) got always-visible inline helper sentences instead of hover tips. Plan-checker caught 3 mistargeted surfaces pre-execution (voice/approval null-publisher+read-only duplicates, signal-desk copy describing story-brief's lead mechanic) — retargeted at VoicePassRail/DecisionRail/LeadActions. 260721-pmn guardrail held (zero setPanelContent routing, module-constant props). Executor stalled at SUMMARY step after completing all 3 commits + gates; orchestrator re-verified independently. Full suite 1081 passed (135 files); strict build clean; verifier 11/11 must-haves. | 2026-07-22 | 8a2593e | Verified | [260722-fom-add-help-tooltips-inline-helper-text-acr](./quick/260722-fom-add-help-tooltips-inline-helper-text-acr/) |
| 260722-n5r | Redesign Issue Workspace stage pages layout (client: "overwhelming for new users, scrolling looks weird"): widened the frame max-w-[1200px]→[1600px] with a 232/minmax(0,1fr)/320 items-start grid; stage-tab nav now sticky top-0 and both side rails (WorkspaceOutline + ContextPanel) sticky/independently-scrolling at lg:; galley display type switched from viewport clamp (52px in a ~360px column = one-word-per-line headlines) to container-query cqi units on .galley-root (@supports-guarded, viewport fallback kept); removed Draft's duplicate nested lg:w-64 SectionChipList column (WorkspaceOutline already owns galley jump-nav) — chip strip now edit-mode-only via new orientation="horizontal" prop, galley gets full canvas width; Fact Check's flat 132-claim wall grouped into collapsible per-section groups (EDITABLE_SECTIONS order + trailing "Other", all-checked collapsed by default, reuses deriveClaimChipState); Approval DecisionRail constrained to max-w-[760px]. One test assertion scoped with within() for the new group-header counts. Full suite 1085 passed; strict build clean. | 2026-07-22 | 9d50a80 | | [260722-n5r-redesign-issue-workspace-stage-pages-lay](./quick/260722-n5r-redesign-issue-workspace-stage-pages-lay/) |
