...
async def test_patch_section_scoped():
    """PATCH /issues/{run_id}/sections/{section_name} only touches the
    scoped section's body — sibling sections are byte-unchanged."""


@pytest.mark.skip(reason="Wave 2/3 — Plan 31-02/03")
async def test_patch_revision_mismatch():
    """A stale ifRevisionID → Sanity 409 → endpoint re-raises 409
